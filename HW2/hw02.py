...
Original file is located at
    https://colab.research.google.com/drive/13JywEl6_TdmbPR4_75w3KN16cURBq43w
...
def load_feat(path):
    feat = torch.load(path) #加载feat的pt文件
    return feat
...
# 这个变换过程有点复杂，先将二维转换成三维，变换完成后又降成二维，自己找数据测一下就行
# 这个方法很巧妙！！！
def concat_feat(x, concat_n): # #传入feat和要拼的frame的数
    assert concat_n % 2 == 1 # n must be odd
    if concat_n < 2: # 则不拼接
        return x

    seq_len, feature_dim = x.size(0), x.size(1) # 行数表示有多少frame，列数表示frame特征数
...
def preprocess_data(split, feat_dir, phone_path, concat_nframes, train_ratio=0.8, train_val_seed=1337):
    ...
    for i, fname in tqdm(enumerate(usage_list)):
        feat = load_feat(os.path.join(feat_dir, mode, f'{fname}.pt')) # size(feat)=(n,39)
        cur_len = len(feat) # 当前audio feature中有多少个frame
        # 得到以每个frame为中心，扩展了concat_nframes个邻近frame的向量。如果一个frame在边缘，则以他自身代替邻近frame进行扩展
...
for epoch in range(num_epoch):
    ...
    for i, batch in enumerate(tqdm(train_loader)):
        ...
        _, train_pred = torch.max(outputs, 1) #返回每个语音（数据）的预测概率值最大的值及其索引
